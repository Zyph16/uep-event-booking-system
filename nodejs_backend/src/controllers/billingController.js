...
class BillingController {
    ...
    static async getIncomeStats(req, res) {
        try {
            const { period, facilityId, startDate, endDate } = req.query; // period: 'monthly' | 'annually'
            const stats = await BillingService.getIncomeStats(period || 'monthly', facilityId, startDate, endDate);
            res.json(stats);
        } catch (e) {
