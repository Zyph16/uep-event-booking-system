...
class BillingService {
    static async createBilling(data) {
        ...
        try {
            ...
            const id = await BillingRepository.create(data, connection);

            // Update Booking Status to Billing Generated (Pre-Approved)
            const bookingId = parseInt(data.booking_id);
            await BookingRepository.update(bookingId, { status: Booking.STATUS_PRE_APPROVED }, connection);

            await connection.commit();

            // 🔔 Notify User (After commit to ensure data exists)
            try {
                // Fetch detailed context
                const bookingDetails = await BookingRepository.getBillingContext(bookingId); // Context fetching can be outside transaction for notification
    ...
    static async getIncomeStats(period, facilityId, startDate, endDate) {
        ...
        return await BillingRepository.getIncomeStats(period, facilityId, startDate, endDate);
    }
}
