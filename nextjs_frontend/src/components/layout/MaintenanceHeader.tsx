...
import { LogOut, User as UserIcon, Calendar, UserCircle } from "lucide-react";
        ...
        <div className="header w-full h-[80px] bg-gradient-to-br from-[#1f3c88] to-[#0d2b6b] shadow-md sticky top-0 z-[1000] flex items-center">
            <div className="navs w-full max-w-[1400px] mx-auto flex justify-between items-center px-4 md:px-8 relative">
                ...
                <div className="nav-left flex items-center text-text-main">
                    <Link href="/maintenance/dashboard">
                        <h1 className="text-[2rem] font-bold text-white m-0 p-0 hover:text-gray-200 transition-colors">UEP</h1>
                    </Link>
                </div>
                ...
                <div className="nav-right flex items-center gap-3 md:gap-4">
                    ...
                    <div className="relative">
                        ...
                        >
                            ...
                            <span className="font-medium hidden sm:block">{user?.username || 'Maintenance'}</span>
                        </button>
                        ...
                        {isProfileDropdownOpen && (
                            <div className="absolute right-0 mt-2 w-56 bg-white rounded-md shadow-lg py-1 z-50 animate-in fade-in slide-in-from-top-2 duration-200 border border-gray-100">
                                <div className="px-4 py-3 border-b border-gray-100 bg-gray-50">
                                    ...
                                    <p className="text-xs text-blue-600 font-medium capitalize">{user?.role_name || 'Maintenance'}</p>
                                </div>
            ...
            {isMobileMenuOpen && (
                <div className="absolute top-[80px] left-0 w-full bg-[#0d2b6b] p-4 flex flex-col gap-2 md:hidden shadow-xl border-t border-white/10">
                    <Link href="/maintenance/dashboard" className="text-white py-3 px-4 hover:bg-white/10 rounded">Equipment Dashboard</Link>
                </div>
            )}
        </div>
    );
