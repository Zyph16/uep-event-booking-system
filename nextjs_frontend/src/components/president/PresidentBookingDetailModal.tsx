...
interface PresidentBookingDetailModalProps {
    isOpen: boolean;
    onClose: () => void;
    booking: any;
...
export default function PresidentBookingDetailModal({ isOpen, onClose, booking, onStatusUpdate }: PresidentBookingDetailModalProps) {
    ...
    const canSignRequest = status === "pending";
    const canSignBilling = status.includes("billing generated");
                ...
                <div className="flex-1 overflow-y-auto p-8 bg-[#f1f5f9] no-scrollbar">
                    ...
                    <div className="mb-6 flex flex-wrap justify-between items-center gap-4 no-print">
                        <div className="flex gap-2">
                            {canSignRequest && (
                                <button
                                    onClick={() => onStatusUpdate('approve', 'Are you sure you want to sign this booking request?')}
                                    className="flex items-center gap-2 bg-[#1f3c88] text-white px-6 py-2.5 rounded-xl font-bold text-sm shadow-lg hover:bg-[#152a5f] transition-all"
                                >
                                    <CheckCircle size={18} />
                                    Sign Request
                                </button>
                            ...
                            {canSignBilling && (
                                <button
                                    onClick={() => onStatusUpdate('approve', 'Are you sure you want to sign this billing statement?')}
                                    className="flex items-center gap-2 bg-emerald-600 text-white px-6 py-2.5 rounded-xl font-bold text-sm shadow-lg hover:bg-emerald-700 transition-all"
                                >
                                    <CheckCircle size={18} />
                                    Sign Billing
                                </button>
                            )}
                            {(canSignRequest || canSignBilling) && (
                                <button
                                    onClick={() => onStatusUpdate('reject', 'Are you sure you want to REJECT this booking?')}
                                    className="flex items-center gap-2 bg-red-500 text-white px-6 py-2.5 rounded-xl font-bold text-sm shadow-lg hover:bg-red-600 transition-all"
                                >
                                    <XCircle size={18} />
                                    Reject
                                </button>
                        ...
                        <div className="text-[14px]">
                            ...
                            <div className="mt-16 grid grid-cols-2 gap-y-12 gap-x-12">
                                ...
                                <div className="text-center">
                                    <div className="border-t border-black pt-1 px-4 inline-block min-w-[200px]">
                                        <div className="font-bold lowercase">
                                            {booking.pm_signed ? "SIGNED" : "________________"}
                                        </div>
